...
| `vbe/chap11/` | `linedemo`, `wiredemo` | `chap11/*.c` (`-dVBE_SUPPORT`) | SVGA ports of both chap11 demos at 640×480×256: `linedemo` (random-line wireframe) was the proof of concept for `black11`'s (chap11-13's 3D pipeline) resolution-aware rasterizer and perspective projection; `wiredemo` (interactive PLG object viewer, resolution-scaled `ViewingDistance`) is the first demo to exercise the actual projection path. See `vbe/PORTING.md`. |
| `vbe/chap12/` | `tridemo`, `gourdemo`, `solidemo`, `textdemo` | `chap12/*.c` (`-dVBE_SUPPORT`) | SVGA ports at 640×480×256: `tridemo` (random flat triangles) was the first to exercise `black11`'s triangle-fill rasterizer (`drawTopTriangle`/`drawBottomTriangle` via `drawTriangle2D`), which clips against the resolution-aware `PolyClipMinX`/etc. `gourdemo` (interactive gouraud-shaded triangle, per-vertex intensity) is the first to exercise `drawTriangle2DGouraud`; its fixed screen-space layout is doubled ×(640/320, 480/200) like any other screen-only UI demo. `solidemo` (interactive PLG object viewer, `drawObjectSolid` with backface removal/shading) mirrors `wiredemo`'s port: resolution-scaled `ViewingDistance`. `textdemo` (texture-mapped triangle, four selectable stone/wood/slime/lava textures) is the last to exercise `black11`'s rasterizer set (`drawTriangle2DText`); its `textures.pcx` sprite sheet is a per-cell-rescaled copy (128×128 cells, not a whole-image resize — see Assets in `vbe/PORTING.md`) so `spriteInit`'s cell size doubles along with the screen layout. `black11.c`'s resolution-aware state (`HalfScreenWidth`/`AspectRatio`/clip rect) resyncs itself internally — no demo needs to call anything. |
| `vbe/chap13/` | `sol2demo` | `chap13/sol2demo.c` (`-dVBE_SUPPORT`) | SVGA port of the camera-movement solid-object viewer (moves `ViewPoint`/`ViewAngle` instead of the object itself, via `createWorldToCamera`/`worldToCameraObject`) — otherwise mirrors `solidemo`'s port. Its two stacked `printStringDb` lines needed line spacing doubled (10→20) to fit `font16.bin`'s 16px-tall glyphs without overlapping. This completes VESA ports of all 7 demos across chap11-13, exercising every rasterizer path in `black11.c`'s design (wireframe, flat, gouraud, textured, and both object-movement and camera-movement solid viewers). |
| `vbe/chap14/` | `objects` | `chap14/objects.c` (`-dVBE_SUPPORT`) | SVGA port of the 4-object camera-movement viewer — combines `sol2demo`'s camera pattern with `removeObject`'s bounding-sphere culling test (`OBJECT_CULL_XYZ_MODE`), exercising `black11.c`'s culling path for the first time. Reuses `black11.c` directly (chap14 has no separate engine snapshot). Its bottom-of-screen "Objects Removed" message and per-object index labels are screen-only UI, doubled the same way as `gourdemo.c`'s HUD. |
