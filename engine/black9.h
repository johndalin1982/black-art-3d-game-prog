...
// register indices in 16450/16550 UART
...
#define SERIAL_GPO2         8   // enable interrupt
...
extern void (_interrupt _far *OldSerialIsr)();  // holds old comm port interrupt handler

extern char SerialBuffer[SERIAL_BUFF_SIZE];     // the receive buffer

extern int SerialEnd;       // indexes into receive buffer
extern int SerialStart;
extern int SerialCh;
extern int CharReady;       // current character and ready flag
extern int OldIntMask;      // the old interrupt mask on the PIC
extern int OpenPort;        // the currently open port
extern int SerialLock;      // serial ISR semaphore so the buffer
                            // isn't altered will it is being written
                            // to by the ISR

extern char* ModemStrings[];
