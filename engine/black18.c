...
void removeBackfacesAndShade(ObjectPtr object, int forceColor) {
    ...
    // black17.c by the addition of the parameter forceColor, this is used
    // to force all the polys to a specific color. this is needed for the
    // explosion vaporization effect. pass -1 to skip the override and use
    // the normal lighting computation.
    ...
    for (currPoly = 0; currPoly < object->numPolys; currPoly++) {
        ...
        if (object->polys[currPoly].twoSided == ONE_SIDED) {
            ...
            if (dp > 0) {
                ...
                if (forceColor > -1) {
                    object->polys[currPoly].shade = forceColor;
                    continue;
                }
...
int clipLine(int* x1, int* y1, int* x2, int* y2) {
    ...
    int point1 = 0, point2 = 0;     // tracks if each end point is visible or invisible
    ...
    if (point1 == 0 && point2 == 0) {
        ...
        if (((*x1 < PolyClipMinX) && (*x2 < PolyClipMinX)) ||   // to the left
            ((*x1 > PolyClipMaxX) && (*x2 > PolyClipMaxX)) ||   // to the right
            ((*y1 < PolyClipMinY) && (*y2 < PolyClipMinY)) ||   // above
            ((*y1 > PolyClipMaxY) && (*y2 > PolyClipMaxY))) {   // below
    ...
    if ((point1 == 1) || (point1 == 0 && point2 == 0)) {
        ...
        if (*x2 > PolyClipMaxX) {
            ...
            if (dx != 0) {
                yi = (int)(0.5 + (dy / dx) * (PolyClipMaxX - *x1) + *y1);
            } else {
            ...
            if (dx != 0) {
                yi = (int)(0.5 + (dy / dx) * (PolyClipMinX - *x1) + *y1);
            } else {
                yi = -1;    // invalidate intersection
            }
        }
        ...
        if (*y2 > PolyClipMaxY) {
            ...
            // compute intersection with right edge
            if (dy != 0) {
                xi = (int)(0.5 + (dx / dy) * (PolyClipMaxY - *y1) + *x1);
            } else {
            ...
            if (dy != 0) {
                xi = (int)(0.5 + (dx / dy) * (PolyClipMinY - *y1) + *x1);
            } else {
    ...
    if ((point2 == 1) || (point1 == 0 && point2 == 0)) {
        ...
        if (*x1 > PolyClipMaxX) {
            ...
            // compute intersection with right edge
            if (dx != 0) {
                yi = (int)(0.5 + (dy / dx) * (PolyClipMaxX - *x2) + *y2);
            } else {
            ...
            // compute intersection with left edge
            if (dx != 0) {
                yi = (int)(0.5 + (dy / dx) * (PolyClipMinX - *x2) + *y2);
            } else {
                yi = -1;    // invalidate intersection
            }
        }
        ...
        if (*y1 > PolyClipMaxY) {
            ...
            // compute intersection with right edge
            if (dy != 0) {
                xi = (int)(0.5 + (dx / dy) * (PolyClipMaxY - *y2) + *x2);
            } else {
                xi = -1;    // invalidate inntersection
            }
            ...
            // compute intersection with top edge
            if (dy != 0) {
                xi = (int)(0.5 + (dx / dy) * (PolyClipMinY - *y2) + *x2);
            } else {
                xi = -1;    // invalidate inntersection
            }
        }
    ...
    (void)clipAlways;   // currently unused but kept for parity with book code
...
void drawLine(
    ...
    videoBufferStart = videoBufferStart + ((unsigned int)yo << 6) +
                                          ((unsigned int)yo << 8) +
                                          (unsigned int)xo;
        ...
        for (index = 0; index <= dx; index++) {
            ...
            *videoBufferStart = color;
            ...
            if (error > dx) {
                ...
                // move to next line
                videoBufferStart += yInc;
            }

            // move to the next pixel
            videoBufferStart += xInc;
        }
    } else {
        // draw the line
...
void drawObjectWire(ObjectPtr object, int color) {
    ...
    for (currPoly = 0; currPoly < object->numPolys; currPoly++) {
        ...
        for (currVertex = 0; currVertex < object->polys[currPoly].numPoints - 1; currVertex++) {
            ...
            x1 = object->verticesWorld[vertex].x;
            y1 = object->verticesWorld[vertex].y;
            z1 = object->verticesWorld[vertex].z;

            vertex = object->polys[currPoly].vertexList[currVertex + 1];

            x2 = object->verticesWorld[vertex].x;
            y2 = object->verticesWorld[vertex].y;
            z2 = object->verticesWorld[vertex].z;
