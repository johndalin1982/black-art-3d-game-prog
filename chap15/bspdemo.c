...
// size of the gadgets (buttons) - doubled to match bspbutt.pcx's
// per-cell-rescaled 128x24 frames under VBE_SUPPORT
#ifdef VBE_SUPPORT
#define GADGET_WIDTH            128
#define GADGET_HEIGHT           24
#else
#define GADGET_WIDTH            64
...
// control areas of GUI - doubled ((640/320) for X, (480/200) for Y) under
// VBE_SUPPORT, same as any other chapter's screen-only UI
#ifdef VBE_SUPPORT
#define BSP_MIN_X               0   // the bounding rectangle of the
#define BSP_MAX_X               444 // BSP editor area
#define BSP_MIN_Y               0
#define BSP_MAX_Y               478

#define INTERFACE_MIN_X         452 // the bounding rectangle of the control
#define INTERFACE_MAX_X         638 // button area to the right of editor
#define INTERFACE_MIN_Y         0
#define INTERFACE_MAX_Y         478
#else
#define BSP_MIN_X               0   // the bounding rectangle of the
...
// gadget lists - positions doubled ((640/320) for X, (480/200) for Y)
// under VBE_SUPPORT, same as BSP_MIN_X/etc above
#ifdef VBE_SUPPORT
Gadget BspControls[6] = {
    { 480, 226, CONTROL_ID_CLEAR },
    { 480, 266, CONTROL_ID_DEL_WALL },
    { 480, 307, CONTROL_ID_BUILD_BSP },
    { 480, 348, CONTROL_ID_PRINT_BSP },
    { 480, 389, CONTROL_ID_VIEW },
    { 480, 430, CONTROL_ID_EXIT }
#else
Gadget BspControls[6] = {
...
void main(void) {
...
#ifdef VBE_SUPPORT
        // fixed demo layout, doubled to match the bigger screen - see
        // chap12/gourdemo.c's port
        readoutX = 456, readoutY = 134,
        // sprite top-left to crosshair-center offset - half of PointerSpr's
        // 18x18 size, matching the -9 recentering above
        crosshairOffset = 9;
#else
        readoutX = 228, readoutY = 56,
        crosshairOffset = 4;
#endif
    ...
    // initialize the controls sprite - GADGET_WIDTH/HEIGHT already account
    // for the VBE_SUPPORT-rescaled bspbutt.pcx cell size
    spriteInit((SpritePtr)&ControlsSpr, 0, 0, GADGET_WIDTH, GADGET_HEIGHT, 0, 0, 0, 0, 0, 0);
    ...
    // initialize the pointer sprite - 18x18 under VBE_SUPPORT, matching
    // bsppoint.pcx's rescaled cell size
#ifdef VBE_SUPPORT
    spriteInit((SpritePtr)&PointerSpr, 0, 0, 18, 18, 0, 0, 0, 0, 0, 0);
#else
    ...
    while (!done) {
        ...
        // The DOS mouse driver reports x in a virtual range that's fixed
        // regardless of video mode (hence the book's >>1 to halve it down
        // to mode-13h's 320-wide range) - under VBE_SUPPORT at 640-wide
        // that range already matches the screen, so the divide is dropped.
        // Offsets center the 18x18 pointer (half its size), same
        // properly-centered-at-the-new-size approach chap05/mousetst.c's
        // HammerSprite uses (its own book offset wasn't centered either -
        // this corrects that, it doesn't just rescale the book's number).
#ifdef VBE_SUPPORT
        mouseX = PointerSpr.x = mouseX - 9;
        mouseY = PointerSpr.y = mouseY - 9;
#else
        ...
        if (buttons == MOUSE_LEFT_BUTTON) {
            ...
            if (mouseX >= BSP_MIN_X && mouseX <= BSP_MAX_X &&
                ...
                if (action == ACTION_STARTING_LINE) {
                    ...
                    Lines[TotalLines].sx = mouseX + crosshairOffset;
                    Lines[TotalLines].sy = mouseY + crosshairOffset;
                    ...
                    Lines[TotalLines].ex = mouseX + crosshairOffset;
                    Lines[TotalLines].ey = mouseY + crosshairOffset;
                    ...
                    deleteLine(mouseX + crosshairOffset, mouseY + crosshairOffset);
        ...
        sprintf(buffer, "(%d,%d) ", (mouseX + crosshairOffset) + SCREEN_TO_WORLD_X,
                                    -((mouseY + crosshairOffset) + SCREEN_TO_WORLD_Z));

        printStringDb(readoutX, readoutY, 10, buffer, 0);
