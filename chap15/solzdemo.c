...
void main(int argc, char** argv) {
    ...
    // so the field of view stays the same as the resolution grows (the
    // book leaves this at its 200 default, so only VBE_SUPPORT needs an
    // explicit assignment)
#ifdef VBE_SUPPORT
    ViewingDistance = (int)(200 * ((float)DisplayWidth / 320.0f));
#endif
    ...
    // allocate double buffer and z buffer matching the active display
#ifdef VBE_SUPPORT
    ...
    while (!done) {
        ...
        for (index = 0; index < 4; index++) {
            ...
            if (!removeObject(&TestObjects[index], OBJECT_CULL_XYZ_MODE)) {
                ...
                printStringDb(index * labelSpacing, 0, 10, buffer, 0);
            }
        }
